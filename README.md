...
The **Ticket assistant** is a compact floating chat available throughout the logged-in admin to users who can view tickets. It leaves the page interactive and offers two actions: **Summarize ticket** (an agent handover covering ownership, age/inactivity, work completed, attachment evidence, open questions, and a suggested next action) and **Draft reply** (customer-facing text to review and copy). Opening a saved ticket selects it automatically; elsewhere, find a ticket by title or number. Follow-up messages can refine a summary or reply. Changing tickets starts a new conversation. The chat survives navigation in the same browser tab using session storage, scoped to the logged-in user. It never saves comments, changes tickets, or sends messages to customers.

The server checks ticket permissions and sends the selected ticket's saved details, assignees, timing, and dated comments with authors/IDs to OpenAI. Age and inactivity are calculated on the server; inactivity means time since the latest ticket edit or new comment, not an SLA violation (comment edit times are not tracked). Each PDF is sent as a file input and each PNG/JPEG as an image input through a fresh five-minute private S3 URL. OpenAI reads the files directly; Django does not download them or create a separate uploaded-file registry. Handovers cite comment IDs and PDF page numbers; comment IDs are visible in the admin. Both actions can use attachment evidence. Unsaved edits, separate customer contact fields, and other tickets are not included.

Replies display as plain text. The paid `gpt-6-luna` Responses API uses no reasoning or tools, `store=False`, a 1,000-character message limit, the last six conversation messages, 500 output tokens, a 20-second provider timeout, and no SDK retries. The existing five-per-user/minute and 200-global/day approximate rate limits remain. All attachments are supplied; files are not silently skipped or truncated. Invalid/unavailable files, provider limits, and timeouts return an explicit error. Large or numerous attachments may exceed provider limits, and PDF pages/images increase input-token cost. This ticket-focused scope replaces the brief's form-definition help bonus; form help is not implemented.

## Test and deploy

The suite has nine cases: resolving requires a saved comment, new tickets cannot start resolved, closed tickets cannot reopen (three destinations), stale edits respect a later closure, bulk resolution handles a mixed selection and reports/logs the result, and two assistant tests cover ticket access and the data sent by both actions without writing records. The context test checks ownership, age/inactivity, PDF/image inputs, and exclusion of unrelated ticket files. Tests use local PostgreSQL and an HTTP transport stub for OpenAI; they do not connect to Supabase or the real provider.
...
- **OpenAI `gpt-6-luna`.** A small paid model used with an existing API key. Native PDF/image inputs avoid an OCR service or document-processing pipeline. The 500-token answer cap and 200-requests-per-day limit bound output and request volume; attachment input costs still depend on file contents. The provider integration lives in `ask_ticket_question`.
