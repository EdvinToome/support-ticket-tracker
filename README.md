...
| Ask about accessible admin forms | Yes | Yes | Yes |
...
The **AI form help** widget is a compact floating chat available throughout the logged-in admin to users with domain-model access. It explains what fields mean and how to fill them out, using the actual Django form definitions and model `help_text`. Ticket, Customer, Agent, and Comment pages select their form automatically; elsewhere, use the **Form** selector. Suggested questions cover required fields and how to complete the form. The page remains interactive. Conversations survive navigation within the same browser tab, scoped to the login; changing the form or record starts a fresh conversation.

The server checks model and record permissions, then describes the visible form and its inlines: field names, labels, types, choices, help text, length limits, and required/read-only state. For example, a new ticket's status is read-only, and a Viewer sees all fields as read-only. Saved record values, relationship option lists, and attachments are not sent to OpenAI. Only these definitions and the user's chat messages are sent. The helper has no write access and does not summarize tickets or draft customer replies.

Answers display as plain text. The paid `gpt-6-luna` Responses API uses no reasoning or tools, `store=False`, a 1,000-character question limit, the last six conversation messages, 500 output tokens, a 10-second provider timeout, and no SDK retries. Approximate limits allow five requests per user per minute and 200 globally per day. Missing keys, provider errors, quota limits, and timeouts produce explicit unavailable messages without disrupting the admin.

## Test and deploy

The suite has nine cases: resolving requires a saved comment, new tickets cannot start resolved, closed tickets cannot reopen (three destinations), stale edits respect a later closure, bulk resolution handles a mixed selection and reports/logs the result, and two AI tests cover access permissions and form metadata without saved record values. The metadata test covers all four forms, read-only fields, inline upload help, and Viewer permissions. Tests use local PostgreSQL and an HTTP transport stub for OpenAI; they do not connect to Supabase or the real provider.
...
- **OpenAI `gpt-6-luna`.** A small paid model used with an existing API key. Form metadata gives it the field-specific context required by the brief without sending saved record values. The 500-token answer cap and 200-requests-per-day limit bound output and request volume. The provider integration lives in `ask_form_question`.
